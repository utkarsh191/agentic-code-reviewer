...
interface ESLintFindingsProps {
  findings: ESLintFinding[];
}

const getSeverityClass = (
  severity: ESLintFinding["severity"]
) => {
  switch (severity) {
    case "error":
    ...
    <div className="mt-8">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Static analysis results from ESLint.
        </p>
      </div>
      ...
      {findings.length === 0 ? (
        <div className="rounded-xl border border-green-900 bg-green-950/30 p-5">
          ...
          <p className="mt-2 text-sm text-green-500">
            ESLint did not detect any problems in the changed code.
          </p>
        </div>
      ) : (
